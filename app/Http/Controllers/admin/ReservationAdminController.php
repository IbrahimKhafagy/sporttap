...
class ReservationAdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $playgroundsWithReservations = Playground::whereHas('reservations')->get();

        return view('admin.reservations.reservations',compact('reservations','playgroundsWithReservations'));
    }
    ...
    public function getReservations(Request $request)
    {
        ...
        $ticketsQuery = Reservation::with(['user', 'playground'])
            ->where(function($query) use ($search) {
                $query->whereHas('user', function($query) use ($search) {
                    ...
                    ->orWhereHas('playground', function($query) use ($search) {
            ...
            ->when($playgroundId, function($query) use ($playgroundId) {
                return $query->where('playground_id', $playgroundId);
            })
...
//        }
    ...
    public function orderDetails($id)
    {
        ...
        $media = Media::find($reservation->playground->media_id);
        if ($media) {

            $filePath = "storage/{$reservation->playground->media_id}/{$media->file_name}";
            $fullUrl = asset("{$filePath}");
            $eventf = $reservation->playground;
            $eventf->image = $fullUrl;


            $reservation->playground = $eventf;
        }

        return view('admin.reservations.reservation-details',compact('reservation'));
    }
